...
4. **Failure and recovery:** explain what can go wrong, why it matters, what work
   remains safe, and when the reader must stop.
...
- Prefer exact paths, menu labels, commands, states, and expected results over
  vague instructions.
- Mark information as **current implementation**, **accepted target**,
  **planned work**, or **advisory guidance** when those categories could be
  confused.
...
| [`onboarding/getting-started.md`](onboarding/getting-started.md) | How do I make a new machine ready for work? | Prerequisites, repository preparation, docs tooling, Unity, Rider, first smoke test, Coordination identity, expected results, and setup recovery. | Recurring task execution or detailed Coordination behavior. | Manifests, scripts, Unity settings, `.gitignore`, and current editor UI. |
| [`collaboration/team-workflow.md`](collaboration/team-workflow.md) | How does a normal task move from selection to safe merge? | Worktree review, task choice, announcement, branching, shared-file safety, implementation, verification, handoff, review, and stabilization. | First-machine setup or detailed tool references. | Repository rules, board contract, Git workflow, and verification commands. |
...
tutorials. They record execution, evidence, rationale, and lifecycle state.
Keep them accurate for their role, but do not rewrite historical prose to match
the current teaching style. Correct live links and active file references when
the documentation structure moves.
