...
- Replace the current narrow drop-strip interaction with column-body hit
  testing in `BoardColumn.vue` and `useDragDrop.ts`.
...
- Add a unit-style UI regression for `BoardColumn.vue` that verifies
  whole-column drag handling is attached to the column body, not just the
  “Drop here” slots.
...
- Re-run the existing reorder tests to confirm the new target-index
  calculation does not change ordering semantics.
