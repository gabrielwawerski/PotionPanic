...
- Add a small dev-only watcher path, preferably alongside the existing
...
- Keep static `vitepress build` behavior non-mutating. If index repair is
  needed outside `docs:dev`, rely on explicit tooling rather than build-time
...
- Existing internal plan helpers should continue to own:
  ...
  - archive integration
- No nav changes, ticket schema changes, or public board API changes are part
  of this slice.
